...
class Customlib
{
    ...
    public function __construct()
    {
        ...
        $this->CI->load->model('frontcms_setting_model');
    }
    ...
    public function getFrontCurrentThemeSetting()
    {
        $new_Array = [];
        
        $theme_selected = $this->CI->frontcms_setting_model->get();      

        $theme_colors = front_theme_color();
        $this->CI->load->model('frontcms_setting_model');

        return $theme_colors[$theme_selected->theme];
    }

    public function getCurrentThemeSetting()
    {
        $new_Array = [];
        $admin = $this->CI->session->userdata('admin');
        $student = $this->CI->session->userdata('student');
        ...
        if ($admin && isset($admin['theme'])) {
            $theme_data = $admin['theme'];

            $new_Array = [
                'theme_color' => $theme_data['theme_color'],
                'theme_shadow' => $theme_data['theme_shadow'],
                'theme_background' => $theme_data['theme_background'],
                'theme_content' => $theme_data['theme_content'],
                'theme_type' => $theme_data['theme_type'],
                'theme_navigation' => $theme_data['theme_navigation'],
                'theme_font_color' => $theme_data['theme_font_color'],
            ];
            return $new_Array;
        } elseif ($student && isset($student['theme'])) {
            $theme_data = $student['theme'];
    ...
    public function checkSessionLanguageRTL()
    {
        $admin = $this->CI->session->userdata('admin');

        if ($admin) {
            return (($admin['is_rtl']) == "disabled") ?  0 : 1;
        } else if ($this->CI->session->userdata('student')) {
            $student = $this->CI->session->userdata('student');

            return (($student['is_rtl']) == "disabled") ?  0 : 1;
        }
    }

    public function getSessionLanguageShortCode()
    {
        $admin = $this->CI->session->userdata('admin');

        if ($admin) {
            $language        = $admin['language'];
            $lang_short_code         = $language['lang_short_code'];
        } else if ($this->CI->session->userdata('student')) {
            $student = $this->CI->session->userdata('student');
            $language        = $student['language'];
            $lang_short_code         = $language['lang_short_code'];
        }
        return (!IsNullOrEmptyString($lang_short_code)) ? $lang_short_code : 'en';
    }


    public function getLanguagePDFJson()
    {

        $language_key = $this->getSessionLanguageShortCode();
        $filePath = "./backend/json-files/datatable_language_font.json";
        ...
        if (file_exists($filePath)) {
            ...
            $data = json_decode($jsonContent, true);  // Pass true for an associative array, false for an object

            // Check if the decoding was successful
            if (json_last_error() === JSON_ERROR_NONE) {

                // Successfully decoded the JSON, you can use $data                
                $set_lang = array_key_exists($language_key, $data['languages']) ? $language_key : 'en';

                if ($set_lang == "en") {

                    $lang_arr_json['en'] = $data['languages']['en'];
                } else {
                    $lang_arr_json[$set_lang] = $data['languages'][$set_lang];
                    $lang_arr_json['en'] = $data['languages']['en'];
                }

                $lang_array_pdf = [
                    'pdf_language_font' => $set_lang,
                    'lang_arr_json' => $lang_arr_json,
                ];
                return $lang_array_pdf;
            }
        }
    }
    ...
    public function getPrintHeaderImage()
    {
        ...
        if ($admin) {
            $headerimage =  $admin['print_heder_image'];
        } else if ($this->CI->session->userdata('student')) {
            $headerimage = $student['print_heder_image'];
        }
            ...
            foreach ($notifications as $note_key => $note_value) {
                if ($note_value->type == $find) {
                    return array(
                        ...
                        'whatsapp_template_id'  => $note_value->whatsapp_template_id,
    ...
    public function get_active_currency_count()
    {
        $this->CI->load->model('currency_model');
        $total_rows = $this->CI->currency_model->get_active_currency_count();
        return $total_rows;
    }

    public function get_active_language_count()
    {
        $this->CI->load->model('language_model');
        $total_rows = $this->CI->language_model->get_active_language_count();
        return $total_rows;
    }

    public function getfrontcmssetting()
    {
        $this->CI->load->model('frontcms_setting_model');
        return $front_setting = $this->CI->frontcms_setting_model->get();
    }

    public function checkfieldexist_transfer_certificate($fieldname)
    {
        $this->CI->load->model('transfercertificate_model');
        $status = $this->CI->transfercertificate_model->getfieldstatus($fieldname);
        return $status;
    }
    ...
    {
        $this->CI->load->model('transfercertificate_model');
        $position = $this->CI->transfercertificate_model->getfieldsposition($fieldname);
        return $position;
    }

    public function get_chat_msg_count()
    {
        $this->CI->load->model('chatuser_model');
        $userdata  = $this->getUserData();
        $userid   = $userdata['id'];
        return  $total = $this->CI->chatuser_model->get_chat_msg_count($userid);
    }

    public function get_student_parent_chat_msg_count()
    {
        $this->CI->load->model('chatuser_model');
        $user_type = $this->CI->session->userdata['student']['role']; ///added
        $student_or_parent_id = $this->CI->customlib->getStudentSessionUserID();
        return  $total = $this->CI->chatuser_model->get_student_parent_chat_msg_count($student_or_parent_id, "$user_type");
    }

    public function gettcfieldcustomstatus($fieldname)
    {
