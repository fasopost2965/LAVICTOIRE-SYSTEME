...
class Qrsetting extends MY_Addon_QRAttendanceController {
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('sub_menu', 'admin/qrattendance/qrsetting/index');
		...
		$data['machine_type'] = $this->qrsetting_model->getmachinetype(); 
        ...
        $this->form_validation->set_rules('machine_type[]', $this->lang->line("scanner_device_type"), 'required|trim|xss_clean');      
        ...
        if ($this->form_validation->run() === false) {
            ...
            $data_insert = array(
				...
				'machine_type'      => json_encode($this->input->post('machine_type')),
            );

            $this->qrsetting_model->add($data_insert);
            $this->session->set_flashdata('msg', '<div class="alert alert-success">' . $this->lang->line('update_message') . '</div>');
            redirect('admin/qrattendance/qrsetting');
        }
    }
