...
class Studentfee extends Admin_Controller
{
    ...
    public function feesearch()
    {
        ...
        if (!empty($currentsessiontransportfee)) {
            ...
            if ($module['is_active']) {
                ...
                $feesessiongroup[count($feesessiongroup)] = (object)array('id' => $this->lang->line('transport'), 'group_name' => $this->lang->line('transport_fees'), 'is_system' => 0, 'feetypes' => $transportfesstype);
            }
        }
    ...
    public function addfee($id)
    {
		...
		if(!empty($this->session->userdata('session_array'))){		
			...
			$current_session_id	=	$this->session->userdata('admin')['session_array'];
		}
    ...
    public function addstudentfee()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $token = encode_receipt_url($invoice_id, $fee_category, $transport_fees_id, $fee_groups_feetype_id, $student_fees_master_id, $fee_session_group_id, 'staff', $staff_record['id']);
            ...
            $mailsms_array->fee_receipt_url    = $fee_receipt_pdf_url;
    ...
    public function addfeegrp()
    {
        ...
        $this->form_validation->set_rules('total_paying', $this->lang->line('paying_amount'), 'required|trim|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            $data = array(
                ...
                'total_paying' => form_error('total_paying'),
            );
			...
			foreach ($total_row as $total_row_key => $total_row_value) {				
				if($this->input->post('fee_amount_' . $total_row_value) > 0){
				    ...
				    $student_transport_fee_mail 		= $this->input->post('trans_fee_id_' . $total_row_value);				 
				
				    $fee_groups_feetype_mail   		= $this->input->post('fee_groups_feetype_id_' . $total_row_value);
				    $student_fees_master_mail   	= $this->input->post('student_fees_master_id_' . $total_row_value);
				    $fee_session_group_mail  		= $this->input->post('fee_session_group_id_' . $total_row_value);				 
