...
class User extends Student_Controller
{
    ...
    public function profile()
    {
        ...
        // ------------- CBSE Exam End---------------------
        $data['googledrivepicketsetting']   = $this->student_model->getgoogledrivepickersetting();
        $this->load->view('layout/student/header', $data);
    ...
    public function user_language($lang_id)
    {
        ...
        $language_array      = array('lang_id' => $lang_id, 'lang_short_code' => $setting_result[0]['language_code'], 'language' => $language_name['language']);
    ...
    public function getfees()
    {
        ...
        if (!empty($this->session->userdata('session_array'))) {
            $current_session_id    =    $this->session->userdata('session_array');
        } else {
            $current_session_id    =    $this->session->userdata('student')['session_array'];
        }

        $display_previous_fees    =    $data['sch_setting']->display_previous_fees;

        $student_session       = $this->student_model->getStudentSessions($student['id'], $display_previous_fees);
        ...
        foreach ($student_session  as $student_session_key => $student_session_value) {

            $fees_array[] = [
                'session_id' => $student_session_value->session_id,
                'session' => $student_session_value->session,
                'fees' => $this->studentfeemaster_model->getStudentFees($student_session_value->current_student_session_id),
                'transport_fees' => $this->studentfeemaster_model->getStudentTransportFees($student_session_value->current_student_session_id, $student_session_value->route_pickup_point_id),
                'student_discount_fee'  => $this->feediscount_model->getStudentFeesDiscount($student_session_value->current_student_session_id)
            ];
        }

        $data['session_fees']            = $fees_array;
    ...
    public function create_doc()
    {
        ...
        $this->form_validation->set_rules('first_doc', $this->lang->line('documents'), 'callback_multihandle_upload[files]');
        ...
        if ($this->form_validation->run() == false) {
            ...
            try {
                ...
                if ($uploaded_files['uploaded'] && is_array($uploaded_files)) {
    ...
    public function edit()
    {
        ...
        $this->form_validation->set_rules('image', $this->lang->line('image'), 'callback_edit_handle_upload[image]');
    ...
    public function printStudentDetails()
    {
        ...
        $marks_division        = $this->marksdivision_model->get();
        $data['student']       = $this->student_model->getStudentByClassSectionIDWithCustomfield($student_current_class->class_id, $student_current_class->section_id, $student_id, $student_current_class->session_id);
        $data['sch_setting']                   =    $this->sch_setting_detail;
        $data['category_list']  = $this->category_model->get();
    ...
    public function addstudentfee()
    {
        ...
        $this->form_validation->set_rules('paying_amount', $this->lang->line('paying_amount'), 'required|trim|xss_clean|numeric|callback_check_deposit');
        if ($this->form_validation->run() == false) {
            $data = array('fees_amount' => form_error('paying_amount'));
            $array = array('status' => 'fail', 'error' => ($data));
            echo json_encode($array);
        } else {
    ...
    public function check_deposit()
    {
        ...
        if (is_numeric($paying_amount)) {
            // Check if paying amount is valid
            if ($paying_amount != "" && $paying_amount != null) {
                if ($paying_amount < 0) {
                    ...
                    $remain_data   = json_decode($remain_amount);
                    $allowed_total = 0;
                    if (isset($remain_data->balance)) {
                        $allowed_total += $remain_data->balance;
                    }
                    if (isset($remain_data->fine_amount)) {
                        $allowed_total += $remain_data->fine_amount;
                    }
                    // Check if paying amount is greater than remaining amount
                    if (convertBaseAmountCurrencyFormat($allowed_total) < $paying_amount) {
                ...
                $this->form_validation->set_message('check_deposit', $this->lang->line('paying_amount') . ' ' . $this->lang->line('is_required'));
                return false;
            }
        } else {
