...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fee_session_group_id = null, $category = null, $gender = null, $rte = null)
    {
        ...
        $teacher_condition = $this->buildTeacherClassSectionCondition('student_session');
        if ($teacher_condition !== '') {
            $sql .= " AND " . $teacher_condition;
        }
...
public function getFeeSessionGroupId($student_fees_master_id)
{
    ...
    return $result ? $result->fee_session_group_id : null;
}
    ...
    public function fee_deposit($data, $send_to, $fee_discounts,$date=null)
    {
    ...
    public function getFeeCollectionReport($start_date, $end_date, $feetype_id = null, $received_by = null, $group = null, $class_id = null, $section_id = null)
    {
        ...
        $module = $this->module_model->getPermissionByModulename('transport');
        if ($module['is_active']) {
            $this->db->select('`student_fees_deposite`.*,students.firstname,students.middlename,students.lastname,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,"Transport Fees" as name, "Transport Fees" as `type`, 0 as is_system,student_transport_fees.student_session_id,students.admission_no,transport_feemaster.month as `code')->from('student_fees_deposite');
        ...
        $result_value2 = $this->filterRowsByTeacherClassSection($result_value2);
	...
	public function getFeeByInvoice($invoice_id, $sub_invoice_id)
    {
        ...
        if (!empty($class_section_array)) {
            ...
            if (!empty($class_section_conditions)) {
                $class_section_where = "(" . implode(' OR ', $class_section_conditions) . ")";
            }
        }
        ...
        if (empty($type['student_transport_fee_id'])) {
            ...
            $this->db->join('students', 'students.id=student_session.student_id');
            $this->db->where('student_fees_deposite.id', $invoice_id);
            if (!empty($class_section_where)) {
             $this->db->where($class_section_where);
            }
            $q = $this->db->get();
    ...
    {
         $sql = "SELECT `fee_groups_feetype`.`fine_type`,fee_groups.is_system,student_fees_master.amount as `student_fees_master_amount`, fee_groups.name as `fee_group_name`,feetype.code as `fee_type_code`,fee_groups_feetype.amount,fee_groups_feetype.fine_percentage,fee_groups_feetype.fine_amount,fee_groups_feetype.due_date,IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` from student_fees_master
    ...
    public function getPreviousStudentFees($student_session_id)
    {
        ...
        if (!empty($normal)) {
            foreach ($normal as $row) {
                ...
                $row->fees = $this->getDueFeeByFeeSessionGroup(
                    $row->fee_session_group_id,
                    $row->id
                );

                if ($row->is_system != 0 && isset($row->fees[0])) {
                    $row->fees[0]->amount = $row->amount;
                }
        ...
        if (!empty($module['is_active'])) {
            ...
            $sql = "SELECT student_transport_fees.*,
                        route_pickup_point.fees AS amount,
                        student_fees_deposite.amount_detail
                    FROM student_transport_fees
                    INNER JOIN route_pickup_point 
                        ON route_pickup_point.id = student_transport_fees.route_pickup_point_id
                    LEFT JOIN student_fees_deposite 
                        ON student_fees_deposite.student_transport_fee_id = student_transport_fees.id
                    WHERE student_transport_fees.student_session_id = " . (int)$student_session_id;
            ...
            if (!empty($transport)) {
                foreach ($transport as $t) {
                    $obj = new stdClass();
                    $obj->name = 'Transport Fees';
                    $obj->fees = array((object)[
                        'amount' => (float)$t->amount,
                        'amount_detail' => $t->amount_detail ?: json_encode([])
                    ]);
                    $result[] = $obj;
                }
            }
	...
	public function deleteBalanceFees($id)
    {
	...
	public function getBalanceMasterRecordNEw($group_name, $student_session_array)
    {
        $sql = "select * from student_fees_master where student_session_id = $student_session_array and fee_session_group_id=(SELECT id FROM `fee_session_groups` where fee_groups_id=(SELECT id FROM `fee_groups` WHERE name=" . "'" . $group_name . "'" . ") and session_id=$this->current_session)";
    ...
    protected function getTeacherAllowedClassSections()
    {
        ...
        foreach ($matrix as $class_id => $sections) {
            ...
            foreach ($sections as $section_id) {
                $map[(int)$class_id][(int)$section_id] = true;
            }
        }
    ...
    protected function filterRowsByTeacherClassSection($rows, $classKey = 'class_id', $sectionKey = 'section_id')
    {
        $allowed = $this->getTeacherAllowedClassSections();
        if ($allowed === null) {
            return $rows;
        }

        if ($allowed === array() || empty($rows)) {
            return array();
        }

        $filtered = array();
        foreach ($rows as $row) {
            if ($this->rowMatchesTeacherScope($row, $allowed, $classKey, $sectionKey)) {
                $filtered[] = $row;
            }
        }
    ...
    protected function rowMatchesTeacherScope($row, $allowed, $classKey, $sectionKey)
    {
        if (is_object($row)) {
            $class_id   = isset($row->{$classKey}) ? (int)$row->{$classKey} : null;
            $section_id = isset($row->{$sectionKey}) ? (int)$row->{$sectionKey} : null;
        } else {
            $class_id   = isset($row[$classKey]) ? (int)$row[$classKey] : null;
            $section_id = isset($row[$sectionKey]) ? (int)$row[$sectionKey] : null;
        }
