...
class Student_model extends MY_Model
{
    ...
    public function UpdateNewMethod($data, $session_data, $fee_session_group, $student_session_id, $delete_fee_session_group, $transport_data, $delete_discount_id, $add_discount_id)
    {
        // Start transaction
        $this->db->trans_start();
        ...
        // Update student_session table if exists
        $this->db->where('session_id', $session_data['session_id']);
        $this->db->where('student_id', $session_data['student_id']);
        ...
        // Insert fee_session_group
        if (!empty($fee_session_group)) {
            $data_insert = array();
            foreach ($fee_session_group as $fee_session_value) {
                $data_insert[] = array(
                    'is_system'            => 0,
                    'student_session_id'   => $student_session_id,
                    'fee_session_group_id' => $fee_session_value
                );
            }
            $this->db->insert_batch('student_fees_master', $data_insert);
        }

        // Delete fee_session_group
        ...
        //Insert Fees Discount
        if (!empty($add_discount_id)) {
            $discount_data_insert = array();
            foreach ($add_discount_id as $discount_id_value) {
                $discount_data_insert[] = array(
                    'student_session_id'   => $student_session_id,
                    'fees_discount_id'     => $discount_id_value
                );
            }
            $this->db->insert_batch('student_fees_discounts', $discount_data_insert);
        }

        // Delete Fees Discount
        if (!empty($delete_discount_id)) {
            $this->db->where('student_session_id', $student_session_id);
            $this->db->where_in('fees_discount_id', $delete_discount_id);
        ...
        if (!empty($transport_data)) {
            ...
            foreach ($transport_data as $insert_value) {
                $this->db->where('student_session_id', $student_session_id);
                ...
                if ($q->num_rows() > 0) {
                    ...
                    $insert_value['student_session_id'] = $student_session_id;
                    $this->db->insert('student_transport_fees', $insert_value);
                    $row_insert[] = $this->db->insert_id();
                }
            }

            // Remove old transport fee records not in the current set
            $this->db->where('student_session_id', $student_session_id);
            $this->db->where_not_in('id', $row_insert);
            $this->db->delete('student_transport_fees');
        } else {
            // No transport data, delete all for this student session
            $this->db->where('student_session_id', $student_session_id);
            $this->db->delete('student_transport_fees');
        }
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            log_message('error', 'Transaction failed in UpdateNewMethod');
            return false;
        }
    ...
    public function UpdateNewMethod_old($data, $session_data, $fee_session_group, $student_session_id, $delete_fee_session_group, $transport_data)
    {
        // Start transaction
        $this->db->trans_start();
    ...
    public function addNewMethod($data, $data_setting, $custom_value_array, $student_session, $fee_session_group, $fees_discounts, $transport_feemasters, $is_sibling, $multiclass_data)
    {
        // Start Transaction
        $this->db->trans_start();
        ...
        if (!empty($custom_value_array)) {
            ...
            foreach ($custom_value_array as $insert_key => $insert_value) {
                $custom_value_array[$insert_key]['belong_table_id'] = $student_id;
            }
		...
		$student_session['default_login'] = 1;
        $this->db->insert('student_session', $student_session);
        $student_session_id = $this->db->insert_id();



        if (!empty($multiclass_data)) {


            foreach ($multiclass_data as $array_key => $array_value) {
        ...
        // Insert student fee discounts
        if (!empty($fees_discounts)) {
            $insert_array = [];
            foreach ($fees_discounts as $value) {
                $insert_array[] = array(
                    'student_session_id' => $student_session_id,
                    'fees_discount_id'   => $value,
                );
            }
            $this->db->insert_batch('student_fees_discounts', $insert_array);
        }

        // Insert transport fees
        if (!empty($transport_feemasters)) {
            foreach ($transport_feemasters as $key => $value) {
                $transport_feemasters[$key]['student_session_id'] = $student_session_id;
            }
            $this->db->insert_batch('student_transport_fees', $transport_feemasters);
        }

        // Insert student login
        $user_password = $this->role->get_random_password(6, 6, false, true, false);
        $data_student_login = array(
            'username' => $this->student_login_prefix . substr($this->setting_model->getCurrentSessionName(), 2, 2) . $data['admission_no'],
        ...
        // Insert parent login
        if ($is_sibling > 0) {
            $student_sibling = $this->student_model->get($is_sibling);
            ...
            $data_parent_login = array(
                'username' => $this->parent_login_prefix . substr($this->setting_model->getCurrentSessionName(), 2, 2) . $data['admission_no'],
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            return json_encode([
                ...
                'data'    => [
                    'student_id'         => $student_id,
                    'student_session_id' => $student_session_id,
                    'student_username'   => $data_student_login['username'],
    ...
    public function searchByClassSection($class_id = null, $section_id = null)
    {
        ...
        $this->db->select('classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no , students.roll_no,students.admission_date,students.firstname,students.middlename,students.lastname,students.image,  students.mobileno,students.email,students.state,students.city,students.pincode,students.religion,students.dob ,students.current_address,students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code, students.guardian_name, students.guardian_relation,students.guardian_phone,students.guardian_email,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.app_key,students.parent_app_key,students.rte,students.gender,vehicles.vehicle_no,transport_route.route_title,route_pickup_point.id as `route_pickup_point_id`,pickup_point.name as `pickup_point`,' . $field_variable)->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
    ...
    public function student_ratio()
    {
        ...
        $this->db->select(' count(*) as total_student, SUM(CASE WHEN students.gender = "Male" THEN 1 ELSE 0 END) AS "male",SUM(CASE WHEN students.gender = "Female" THEN 1 ELSE 0 END) AS "female", classes.class,sections.section, classes.id as class_id, sections.id as section_id')->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
        ...
        foreach ($matrix as $class_id => $sections) {
            foreach ($sections as $section_id) {
                $allowed[$class_id][$section_id] = true;
            }
        }

        return array_values(array_filter($result, function ($row) use ($allowed) {
            $class_id   = isset($row['class_id']) ? (int)$row['class_id'] : 0;
            $section_id = isset($row['section_id']) ? (int)$row['section_id'] : 0;
            return isset($allowed[$class_id][$section_id]);
        }));
    }
    ...
    {
        $array = implode(',', $array);
        $sql   = ' SELECT students.*,student_session.id as student_session_id FROM students INNER join (SELECT * FROM `student_session` WHERE `student_session`.`id` IN (' . $array . ')) as student_session on students.id=student_session.student_id';
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    public function addMultipleDocs($files)
    {
        $this->db->insert_batch('student_doc', $files);
    }
    ...
    public function searchByClassSectionWithSession($class_id = null, $section_id = null, $session_id = null)
    {
        ...
        $allowed = array();
        foreach ($matrix as $class => $sections) {
            foreach ($sections as $section) {
                $allowed[(int)$class][(int)$section] = true;
            }
        }
        ...
        return array_values(array_filter($result, function ($row) use ($allowed) {
            ...
            return $class_id !== null && $section_id !== null && isset($allowed[$class_id][$section_id]);
        }));
    }
    ...
    public function getStudentSessions($student_id, $display_previous_fees)
    {
        $where_condition = [];
        $where_condition[] = "student_session.student_id=" . $student_id;

        if (!$display_previous_fees) {
            $where_condition[] = "student_session.session_id=" . $this->current_session;;
        }
        $where_clause = implode(" AND ", $where_condition);

        $sql   = "SELECT sessions.session,sessions.id as session_id,student_session.route_pickup_point_id,student_session.vehroute_id,student_session.student_id as student_id, student_session.id as current_student_session_id, student_session.class_id as current_session_class_id ,students.firstname,students.middlename,students.lastname,students.admission_no,students.roll_no,students.father_name,students.admission_date FROM `student_session` INNER join sessions on student_session.session_id=sessions.id  INNER join students on students.id =student_session.student_id  JOIN `classes` ON `student_session`.`class_id` = `classes`.`id` JOIN `sections` ON `sections`.`id` = `student_session`.`section_id` where " . $where_clause . " ORDER BY student_session.session_id ASC";
    ...
    public function disablestudentByClassSection($class_id, $section_id)
    {
        ...
        if (!empty($custom_fields)) {
            foreach ($custom_fields as $custom_fields_key => $custom_fields_value) {
                ...
                $join_array .= " LEFT JOIN `custom_field_values` as `" . $tb_counter . "` ON `students`.`id` = `" . $tb_counter . "`.`belong_table_id` AND `" . $tb_counter . "`.`custom_field_id` = " . $custom_fields_value->id;


                array_push($field_var_array_name, $custom_fields_value->name);
                $i++;
            }
        }
        $field_variable = (empty($field_var_array)) ? "" : "," . implode(',', $field_var_array);
        ...
        if ($class_id != null) {
            ...
            $class_section_ary[] = 'student_session.class_id=' . $class_id;
        }
        if ($section_id != null) {
            $class_section_ary[] = 'student_session.section_id=' . $section_id;
        }

        $where_cls_clause = 'and ' . implode(' and ', $class_section_ary);

        $class_section_where = "";
        if (!empty($class_section_array)) {
            $class_section_conditions = array();
            foreach ($class_section_array as $class_sectionkey => $class_sectionvalue) {
                foreach ($class_sectionvalue as $class_sectionvaluekey => $class_sectionvaluevalue) {
                    $class_section_conditions[] = "(`student_session`.`class_id` = " . (int)$class_sectionkey . " AND `student_session`.`section_id` = " . (int)$class_sectionvaluevalue . ")";
                }
            }
            if (!empty($class_section_conditions)) {
                $class_section_where = " AND (" . implode(' OR ', $class_section_conditions) . ")";
            }
        }
        $sql = "select * from (SELECT (SELECT GROUP_CONCAT(CONCAT(classes.class, '(', sections.section, ')') SEPARATOR ', ') FROM student_session JOIN classes ON student_session.class_id = classes.id JOIN sections ON sections.id = student_session.section_id  WHERE student_session.session_id = " . $this->current_session . " $class_section_where  AND student_session.student_id = students.id) AS class_section_list,`classes`.`id` AS `class_id`, `student_session`.`id` as `student_session_id`, `classes`.`class`, `sections`.`id` AS `section_id`, `sections`.`section`, `students`.`id`, `students`.`admission_no`, `students`.`roll_no`, `students`.`admission_date`, `students`.`firstname`, `students`.`middlename`, `students`.`lastname`, `students`.`image`, `students`.`mobileno`, `students`.`email`, `students`.`state`, `students`.`city`, `students`.`pincode`, `students`.`religion`, DATE(students.dob) as dob, `students`.`current_address`, `students`.`permanent_address`, IFNULL(students.category_id, 0) as `category_id`, IFNULL(categories.category, '') as `category`, `students`.`adhar_no`, `students`.`samagra_id`, `students`.`bank_account_no`, `students`.`bank_name`, `students`.`ifsc_code`, `students`.`father_name`, `students`.`guardian_name`, `students`.`guardian_relation`, `students`.`guardian_phone`, `students`.`guardian_address`, `students`.`is_active`, `students`.`created_at`, `students`.`updated_at`, `students`.`gender`,`students`.`dis_reason`,`students`.`dis_note`, `students`.`rte`, `student_session`.`session_id`  " . $field_variable . " FROM `students` JOIN `student_session` ON `student_session`.`student_id` = `students`.`id` JOIN `classes` ON `student_session`.`class_id` = `classes`.`id` JOIN `sections` ON `sections`.`id` = `student_session`.`section_id` JOIN `class_sections` ON `class_sections`.`class_id` = `classes`.`id` and `class_sections`.`section_id` = `sections`.`id` LEFT JOIN `categories` ON `students`.`category_id` = `categories`.`id` LEFT JOIN `school_houses` ON `students`.`school_house_id` = `school_houses`.`id` " . $join_array . " WHERE `student_session`.`session_id` = " . $this->current_session . " AND `students`.`is_active` = 'no' " . $where_cls_clause . " GROUP BY  students.id ORDER BY students.id DESC ) as new_students";
    ...
    public function disablestudentFullText($searchterm)
    {
        ...
        $find_string = 'students.firstname,students.middlename,students.lastname,school_houses.house_name,students.guardian_name,students.adhar_no,students.samagra_id,students.roll_no,students.admission_no,students.mobileno,students.email,students.religion,students.cast,students.gender,students.current_address,students.permanent_address,students.blood_group,students.bank_name,students.ifsc_code,students.father_name,students.father_phone,students.father_occupation,students.mother_name,students.mother_phone,students.mother_occupation,students.guardian_name,students.guardian_relation,students.guardian_phone,students.guardian_occupation,students.guardian_address,students.guardian_email,students.previous_school,students.note';


        $fields = explode(',', $find_string);

        // Use array_map to create "field LIKE '%searchterm%'" for each field
        $like_clauses = array_map(function ($field) use ($searchterm) {
            return $field . " LIKE '%" . addslashes($searchterm) . "%'";
        }, $fields);
        // Join all conditions with OR
        $where_find_clause = " and (" . implode(' OR ', $like_clauses) . ")";
    ...
    public function getStudentsByArray($array)
    {
        ...
        $this->db->select('classes.id AS `class_id`,student_session.id as student_session_id,students.id,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no,students.roll_no,students.admission_date,students.firstname, students.middlename,students.lastname,students.image,students.mobileno,students.email,students.state,students.city,students.pincode,students.religion,     students.dob ,students.current_address,students.blood_group,students.permanent_address,IFNULL(students.category_id, 0) as `category_id`,IFNULL(categories.category, "") as `category`,students.adhar_no,students.samagra_id,students.bank_account_no,students.cast,students.bank_name, students.ifsc_code,students.guardian_name, students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.mother_name,students.updated_at,students.father_name,students.rte,students.gender,users.id as `user_tbl_id`,users.username,users.password as `user_tbl_password`,users.is_active as `user_tbl_active`,school_houses.house_name,' . $field_variable)->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
        $this->db->join('categories', 'students.category_id = categories.id', 'left');
    ...
    public function student_profile($condition1, $condition2)
    {
        ...
        $this->db->select('student_session.transport_fees,student_session.vehroute_id,vehicle_routes.route_id,vehicle_routes.vehicle_id,transport_route.route_title,vehicles.vehicle_no,hostel_rooms.room_no,vehicles.driver_name,vehicles.driver_contact,hostel.id as `hostel_id`,hostel.hostel_name,room_types.id as `room_type_id`,room_types.room_type ,students.hostel_room_id,student_session.id as `student_session_id`,student_session.fees_discount,classes.id AS `class_id`,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no, students.roll_no,students.admission_date,students.firstname,students.middlename,students.lastname,students.image,students.mobileno,students.email ,students.state,students.city,students.pincode,students.note, students.religion, students.cast,school_houses.house_name,students.dob,students.current_address,students.previous_school,
            students.guardian_is,students.parent_id,            students.permanent_address,students.category_id,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name,students.ifsc_code,students.guardian_name,students.father_pic ,students.height ,students.weight,students.measurement_date, students.mother_pic,students.guardian_pic , students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active ,students.created_at ,students.updated_at,students.father_name,students.father_phone,students.blood_group,students.school_house_id,students.father_occupation,students.mother_name,students.mother_phone,students.mother_occupation,students.guardian_occupation,students.gender,students.guardian_is,students.rte,students.guardian_email, users.username,users.password,students.dis_reason,students.dis_note,category,' . $field_variable)->from('students');
    ...
    public function student_profile_old($condition1, $condition2)
    {
        $this->db->select('student_session.transport_fees,student_session.vehroute_id,vehicle_routes.route_id,vehicle_routes.vehicle_id,transport_route.route_title,vehicles.vehicle_no,hostel_rooms.room_no,vehicles.driver_name,vehicles.driver_contact,hostel.id as `hostel_id`,hostel.hostel_name,room_types.id as `room_type_id`,room_types.room_type ,students.hostel_room_id,student_session.id as `student_session_id`,student_session.fees_discount,classes.id AS `class_id`,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no, students.roll_no,students.admission_date,students.firstname,students.middlename,students.lastname,students.image,students.mobileno,students.email ,students.state,students.city,students.pincode,students.note, students.religion, students.cast,school_houses.house_name,students.dob,students.current_address,students.previous_school,
    ...
    public function searchdtByClassSection($class_id = null, $section_id = null)
    {
        ...
        $sql = "select * from (SELECT (SELECT GROUP_CONCAT(CONCAT(classes.class, '(', sections.section, ')') SEPARATOR ', ') FROM student_session JOIN classes ON student_session.class_id = classes.id JOIN sections ON sections.id = student_session.section_id  WHERE student_session.session_id = " . $this->current_session . " $class_section_where  AND student_session.student_id = students.id) AS class_section_list,`classes`.`id` AS `class_id`, `student_session`.`id` as `student_session_id`, `classes`.`class`, `sections`.`id` AS `section_id`, `sections`.`section`, `students`.`id`, `students`.`admission_no`, `students`.`roll_no`, `students`.`admission_date`, `students`.`firstname`, `students`.`middlename`, `students`.`lastname`, `students`.`image`, `students`.`mobileno`, `students`.`email`, `students`.`state`, `students`.`city`, `students`.`pincode`, `students`.`religion`, DATE(students.dob) as dob, `students`.`current_address`, `students`.`permanent_address`, IFNULL(students.category_id, 0) as `category_id`, IFNULL(categories.category, '') as `category`, `students`.`adhar_no`, `students`.`samagra_id`, `students`.`bank_account_no`, `students`.`bank_name`, `students`.`ifsc_code`, `students`.`father_name`, `students`.`guardian_name`, `students`.`guardian_relation`, `students`.`guardian_phone`, `students`.`guardian_address`, `students`.`is_active`, `students`.`created_at`, `students`.`updated_at`, `students`.`gender`, `students`.`rte`, `student_session`.`session_id`  " . $field_variable . " FROM `students` JOIN `student_session` ON `student_session`.`student_id` = `students`.`id` JOIN `classes` ON `student_session`.`class_id` = `classes`.`id` JOIN `sections` ON `sections`.`id` = `student_session`.`section_id` JOIN `class_sections` ON `class_sections`.`class_id` = `classes`.`id` and `class_sections`.`section_id` = `sections`.`id` LEFT JOIN `categories` ON `students`.`category_id` = `categories`.`id` LEFT JOIN `school_houses` ON `students`.`school_house_id` = `school_houses`.`id` " . $join_array . " WHERE `student_session`.`session_id` = " . $this->current_session . " AND `students`.`is_active` = 'yes' " . $where_cls_clause . " GROUP BY 
    students.id ORDER BY students.id DESC ) as new_students";
        $this->datatables->query($sql);
        $this->datatables->searchable('new_students.admission_no,new_students.firstname,new_students.middlename,new_students.lastname,new_students.roll_no,new_students.id,new_students.father_name,new_students.dob,new_students.gender,new_students.category,new_students.mobileno,new_students.class_section_list' . $field_name);
        $this->datatables->orderable('new_students.admission_no,new_students.firstname,new_students.middlename,new_students.lastname,new_students.roll_no,new_students.id,new_students.father_name,new_students.dob,new_students.gender,new_students.category,new_students.mobileno,new_students.class_section_list' . $field_name)
            ->sort('new_students.id');
        $this->datatables->query_where_enable(false);
        return $this->datatables->generate('json');
    }
    ...
    public function searchFullText($searchterm, $carray = null)
    {
        ...
        $find_string = 'firstname,middlename,lastname,guardian_name,adhar_no,samagra_id,roll_no,admission_no,mobileno,email,religion,gender,current_address,permanent_address,bank_name,ifsc_code,father_name,guardian_name,guardian_relation,guardian_phone,guardian_address';
        ...
        // Join all conditions with OR
        $where_find_clause = " (" . implode(' OR ', $like_clauses) . ")";
        ...
        $sql = "select * from (SELECT (SELECT GROUP_CONCAT(CONCAT(classes.class, '(', sections.section, ')') SEPARATOR ', ') FROM student_session JOIN classes ON student_session.class_id = classes.id JOIN sections ON sections.id = student_session.section_id  WHERE student_session.session_id = " . $this->current_session . " $class_section_where AND student_session.student_id = students.id) AS class_section_list,`classes`.`id` AS `class_id`, `student_session`.`id` as `student_session_id`, `classes`.`class`, `sections`.`id` AS `section_id`, `sections`.`section`, `students`.`id`, `students`.`admission_no`, `students`.`roll_no`, `students`.`admission_date`, `students`.`firstname`, `students`.`middlename`, `students`.`lastname`, `students`.`image`, `students`.`mobileno`, `students`.`email`, `students`.`state`, `students`.`city`, `students`.`pincode`, `students`.`religion`, DATE(students.dob) as dob, `students`.`current_address`, `students`.`permanent_address`, IFNULL(students.category_id, 0) as `category_id`, IFNULL(categories.category, '') as `category`, `students`.`adhar_no`, `students`.`samagra_id`, `students`.`bank_account_no`, `students`.`bank_name`, `students`.`ifsc_code`, `students`.`father_name`, `students`.`guardian_name`, `students`.`guardian_relation`, `students`.`guardian_phone`, `students`.`guardian_address`, `students`.`is_active`, `students`.`created_at`, `students`.`updated_at`, `students`.`gender`, `students`.`rte`, `student_session`.`session_id`  " . $field_variable . " FROM `students` JOIN `student_session` ON `student_session`.`student_id` = `students`.`id` JOIN `classes` ON `student_session`.`class_id` = `classes`.`id` JOIN `sections` ON `sections`.`id` = `student_session`.`section_id` JOIN `class_sections` ON `class_sections`.`class_id` = `classes`.`id`  and `class_sections`.`section_id` = `sections`.`id` LEFT JOIN `categories` ON `students`.`category_id` = `categories`.`id` LEFT JOIN `school_houses` ON `students`.`school_house_id` = `school_houses`.`id` " . $join_array . " WHERE `student_session`.`session_id` = " . $this->current_session . " $class_section_where AND `students`.`is_active` = 'yes' GROUP BY  students.id ORDER BY students.id DESC ) as new_students where ".$where_find_clause;
        ...
        $this->datatables->orderable('new_students.admission_no,new_students.firstname,new_students.middlename,new_students.lastname,new_students.roll_no,new_students.id,new_students.father_name,new_students.dob,new_students.gender,new_students.category,new_students.mobileno,new_students.class_section_list' . $field_name)
            ->sort('new_students.id');
        $this->datatables->query_where_enable(true);
    ...
    public function searchFullTextAdmin($searchterm, $carray = null)
    {
    ...
    {
        $sql    = "SELECT staff.id,staff.employee_id,staff.name,staff.surname,staff.contact_no,staff.email,'staff' as `table_type`,null as class_section_id,roles.id as staff_role
FROM `staff` inner join staff_roles on staff_roles.staff_id =staff.id INNER JOIN roles on roles.id=staff_roles.role_id
WHERE employee_id=" . $this->db->escape($admission_no) . " 
UNION 
SELECT student_session.id as `student_session_id`,students.id, students.admission_no,students.firstname,students.middlename,students.lastname,'student' as `table_type` ,class_sections.id as `class_section_id`,null as staff_role
FROM `students`
 ...
 inner JOIN class_sections on class_sections.class_id=classes.id and class_sections.section_id=sections.id
 ...
 LEFT JOIN `vehicle_routes` ON `vehicle_routes`.`id` = `student_session`.`vehroute_id` 
 LEFT JOIN `route_pickup_point` ON `route_pickup_point`.`id` = `student_session`.`route_pickup_point_id` 
 LEFT JOIN `pickup_point` ON `route_pickup_point`.`pickup_point_id` = `pickup_point`.`id` 
 ...
 AND `students`.`admission_no` =" . $this->db->escape($admission_no);
    ...
    public function check_parent_mobile_no_exists($str)
    {
        $mobile = $this->security->xss_clean($str);
        if ($mobile != "") {
            $id = $this->input->post('student_id');
            ...
            $isexist = $this->check_parent_mobile_no_data_exists($mobile, $id);
            if ($isexist) {
                $this->form_validation->set_message('check_parent_mobile_exists', $this->lang->line('record_already_exist'));
                return false;
            }
        }
        return true;
    ...
    public function check_parent_mobile_no_data_exists($mobile, $id)
    {
        $this->db->where('guardian_phone', $mobile);
        $this->db->where('id !=', $id);
        $query = $this->db->get('students');
        if ($query->num_rows() > 0) {
            return true;
    ...
    public function checkmulticlassstudentexistornot($id)
    {

        $query = $this->db->query("SELECT COUNT(*) as total from student_session where student_session.student_id = " . $id);

        $row = $query->row_array();
        return $row['total'];
    }

    public function getStudentByClassSectionIDWithCustomfield($class_id = null, $section_id = null, $id = null, $session_id = null)
    {
        if ($session_id != "") {
            $session_id = $session_id;
        } else {
        ...
        $this->db->select('pickup_point.name as pickup_point_name,student_session.route_pickup_point_id,student_session.vehroute_id,vehicle_routes.route_id,vehicle_routes.vehicle_id,transport_route.route_title,vehicles.vehicle_no,hostel_rooms.room_no,vehicles.driver_name,vehicles.driver_contact,hostel.id as `hostel_id`,hostel.hostel_name,room_types.id as `room_type_id`,room_types.room_type,students.hostel_room_id,student_session.id as `student_session_id`,student_session.fees_discount,classes.id AS `class_id`,classes.class,sections.id AS `section_id`,sections.section,students.id,students.admission_no,students.roll_no,students.admission_date,students.firstname, students.middlename,students.lastname,students.image,students.mobileno,students.email,students.state,students.city,students.pincode, students.note, students.religion,students.cast,school_houses.house_name,students.dob,students.current_address,students.previous_school,
            students.guardian_is,students.parent_id,
            students.permanent_address,students.category_id,students.adhar_no,students.samagra_id,students.bank_account_no,students.bank_name, students.ifsc_code,students.guardian_name,students.father_pic,students.height,students.weight,students.measurement_date, students.mother_pic,students.guardian_pic, students.guardian_relation,students.guardian_phone,students.guardian_address,students.is_active,students.created_at ,students.updated_at,students.father_name,students.father_phone,students.blood_group,students.school_house_id,students.father_occupation,students.mother_name,students.mother_phone,students.mother_occupation,students.guardian_occupation,students.gender,students.guardian_is,students.rte,students.guardian_email,sessions.session, users.username,users.password,students.dis_reason,students.dis_note,students.app_key,students.parent_app_key,' . $field_variable)->from('students');
        $this->db->join('student_session', 'student_session.student_id = students.id');
        $this->db->join('sessions', 'sessions.id = student_session.session_id');
        $this->db->join('classes', 'student_session.class_id = classes.id');
        $this->db->join('sections', 'sections.id = student_session.section_id');
    ...
    public function getgoogledrivepickersetting()
    {
        $this->db->select("*")->from('google_drive_setting');
        $this->db->where('id', 1);
        $query = $this->db->get();
        return $query->row_array();
    }

    public function savegoogledrive($data)
    {
        $this->db->trans_start(); # Starting Transaction
        $this->db->trans_strict(false); # See Note 01. If you wish can remove as well
        //=======================Code Start===========================
        if (isset($data['id']) && $data['id'] > 0) {
            $this->db->where('id', $data['id']);
            $this->db->update('google_drive_setting', $data);
            $message = UPDATE_RECORD_CONSTANT . " On google_drive_setting id " . $data['id'];
            $action = "Update";
            $record_id = $data['id'];
            $this->log($message, $record_id, $action);
        } else {
            $this->db->insert('google_drive_setting', $data);
            $insert_id = $this->db->insert_id();
            $message = INSERT_RECORD_CONSTANT . " On google_drive_setting id " . $insert_id;
            $action = "Insert";
            $record_id = $insert_id;
            $this->log($message, $record_id, $action);
        }
    ...
    public function studentUpdateByID($student_id, $data)
    {
    ...
    public function addBulkInsertdoc($data_array)
    {
        if (!empty($data_array)) {
            $this->db->insert_batch('student_doc', $data_array);
            return true;
        }
        return false;
    }
