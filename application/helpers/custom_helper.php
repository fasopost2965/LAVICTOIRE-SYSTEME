...
if (!function_exists('front_theme_color')) {

    function front_theme_color()
    {
...
function amountFormat($amount)
{
    ...
    // $calculated_amount = bcmul((string)$amount, (string)$currency_price, 6);
    $calculated_amount = ($amount * $currency_price);

    if ($currency_format == "#,###.##") {
        $return_amt = number_format($calculated_amount, 2, '.', ',');
        ...
        $return_amt = number_format($calculated_amount, 3, '.', ',');
    } elseif ($currency_format == "####.##") {
        $return_amt = number_format($calculated_amount, 2, '.', '');
    } elseif ($currency_format == "#,##,###.##") {
        $return_amt = indian_money_format($calculated_amount);
    }
    return $return_amt;
}
...
function convertCurrencyFormatToBaseAmount($amount)
{
    ...
    $base_amount = bcdiv((string)$amount, (string)$currency_price, 6);
...
function encode_receipt_url($invoice_id, $fee_category, $transport_fees_id, $fee_groups_feetype_id, $student_fees_master_id, $fee_session_group_id, $type, $created_by)
{
    $data = implode('|', [
        $invoice_id,
        $fee_category,
...
if (!function_exists('check_login_token')) {

    function check_login_token()
    {
        $CI = &get_instance();

        $student_session = $CI->session->userdata('student');

        if (isset($student_session['role']) && $student_session['role'] == 'student') {
            $CI->db->select('login_token');
            $CI->db->from('users');
            $CI->db->where('user_id', $student_session['student_id']);
            $query  = $CI->db->get();
            $result = $query->row();

            // Compare token
            if ($student_session['login_token'] != $result->login_token) {
