...
class Conferencehistory_model extends MY_Model {
    ...
    public function getmeeting() {
        $sql = "SELECT conferences.*,(SELECT COUNT(*) FROM conferences_history WHERE conferences_history.conference_id=conferences.id) as `total_viewers`,`create_by`.`name` as `create_by_name`, `create_by`.`surname` as `create_by_surname`, create_by.employee_id, staff_roles.role_id,create_by.employee_id as create_by_employee_id  FROM `conferences` JOIN `staff` as `create_by` ON `create_by`.`id` = `conferences`.`created_id` JOIN staff_roles on staff_roles.staff_id=`create_by`.`id` WHERE purpose='meeting' and status=2 ORDER BY DATE(`conferences`.`date`) DESC, `conferences`.`date` ASC";
