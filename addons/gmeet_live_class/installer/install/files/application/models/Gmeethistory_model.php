...
class Gmeethistory_model extends MY_Model {
    ...
    public function getmeeting() {
        $sql = "SELECT gmeet.*,(SELECT COUNT(*) FROM gmeet_history WHERE gmeet_history.gmeet_id=gmeet.id) as `total_viewers`,`create_by`.`name` as `create_by_name`, `create_by`.`surname` as `create_by_surname`,create_by.employee_id as create_by_employee_id, staff_roles.role_id  FROM `gmeet` JOIN `staff` as `create_by` ON `create_by`.`id` = `gmeet`.`created_id` JOIN staff_roles on staff_roles.staff_id=`create_by`.`id` WHERE purpose='meeting' and status=2 ORDER BY DATE(`gmeet`.`date`) DESC, `gmeet`.`date` ASC";
