...
?>
                  ...
                  <h4 class="text-center font-weight-bold"><?php  echo $exam->exam.($exam->is_quiz ? " (".$this->lang->line('quiz').")" : " (".$this->lang->line('exam').")"); ?></h4>
                    ...
                    <?php }else if($userdata["role"]=='guest'){ ?>
                        ...
                        <div class="row">
                           ...
                           <div class="col-lg-4 col-md-4 col-sm-12">
                              <div class="row">
                                 <dt class="col-sm-6 col-xs-12 col-md-6 col-lg-6">user type --r</dt>
               ...
               <?php if($submitstatus==0 &&  strtotime(date('Y-m-d H:i:s')) <= strtotime(date($exam->exam_to)) &&  strtotime(date('Y-m-d H:i:s')) >= strtotime(date($exam->exam_from))){   ?>
               ...
               <?php       
               if($submitstatus>0 && !$exam->publish_result){  // exam has been submitted by the student ?>
...
//if ($exam->is_rank_generated && $exam->publish_result) {
      ...
      /} else { ?><?php echo $this->lang->line('awaited') ?><?php
//}
   ?>
  <?php
//}
...
<div class="font-weight-bold"> <?php echo $this->lang->line('attachment') ?>: <a download href="<?php echo site_url('./uploads/course_content/online_course_exam_result/' . $question_value->attachment_upload_name); ?>"><?php echo $question_value->attachment_name; ?> <i class="fa fa-download"></i></a></div>
<?php } ?>
...
<div class="questionmodal">
   ...
   <form id="regiration_form"  action="<?php echo site_url('user/studentcourse/save') ?>"  enctype="multipart/form-data" method="post">
...
<script>
   ...
   $(document).on('click', '.print_div', function () {
        ...
        var id = $this.data('recordId');
               $.ajax({
                    url: "<?php echo site_url('user/studentcourse/print') ?>",
                    type: "POST",
                    data: {'exam_id': id},
                    dataType: 'Json',
