                ...
                <div class="box box-primary" id="route">
                    <div class="box-header ptbnull">
                        ...
                        <div class="pull-right button-gap-mb-1">
							...
							<?php if($this->rbac->hasPrivilege('online_course_question_tag', 'can_view')) { ?>
                                <a class="btn btn-primary btn-sm " href="<?php echo site_url('onlinecourse/coursetag/index') ?>" ><i class="fa fa-plus"></i> <?php echo $this->lang->line('add_tag'); ?></a>
							<?php } ?>
							<?php if($this->rbac->hasPrivilege('online_course_question_bank', 'can_add')) { ?>
							...
							<?php } ?>	
							<?php if($this->rbac->hasPrivilege('online_course_question_bank', 'can_delete')) { ?>
                    ...
                    <div class="box-body">
                        <form role="form" action="<?php echo site_url('onlinecourse/courseexamquestion/questionsearchvalidation') ?>" method="post" class="" id="questionsearchform">
                            <div class="row">
                                ...
                                <div class="col-sm-3 col-md-2">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="question_tag">
                                            ...
                                            <?php
                                                foreach ($question_tag as $key => $val){  ?>
                        ...
                        <table class="table table-striped table-bordered table-hover all-list" data-export-title="<?php echo $this->lang->line('question_bank'); ?>">
                            <thead>
                                <tr>
                                    ...
                                    <th  class="dt-body-left dt-head-left"><?php echo $this->lang->line('q_id'); ?></th>                                    
                                    <th><?php echo $this->lang->line('question_tag'); ?> </th>
                                    <th><?php echo $this->lang->line('question_type') ?></th>
                                    <th><?php echo $this->lang->line('level'); ?></th>
                                    <th><?php echo $this->lang->line('question') ?></th>
                                    <th><?php echo $this->lang->line('created_by') ?></th>
                                    <th class="dt-body-right dt-head-right noExport"><?php echo $this->lang->line('action'); ?></th>
                                </tr>
                            </thead>
                        </table>
                    </div>
...
<div id="myModal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-xl">
        ...
        <div class="modal-content">
            ...
            <form action="<?php echo site_url('onlinecourse/courseexamquestion/add'); ?>" method="POST" id="formsubject">
...
<div id="myQuesImportModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <a class="btn btn-primary pull-right btn-xs mt5 mr-1" data-toggle="tooltip" data-original-title="<?php echo $this->lang->line("download_sample_csv_file"); ?>"  href="<?php echo site_url('onlinecourse/courseexamquestion/exportformat'); ?>" target="_blank"><i class="fa fa-download"></i>  </a>
                <h4 class="modal-title"> <?php echo $this->lang->line('import_question'); ?></h4>
            </div>
            <form action="<?php echo site_url('onlinecourse/courseexamquestion/uploadfile'); ?>" method="POST" id="formimportquestion">
...
$(document).on('submit','#questionsearchform',function(e){
    ...
    $.ajax({
           ...
           data: form_data, // serializes the form's elements.
              ...
              success: function(response) { // your success handler
                if(!response.status){
                   ...
                   initDatatable('all-list', 'onlinecourse/courseexamquestion/getDatatable', response.params,[], 100,
                        [
                            { "bSortable": false, "aTargets": [ 0 ]},
                            { "bSortable": false, "aTargets": [ -1 ] ,'sClass': 'dt-body-right'},
                            { "bSortable": false, "aTargets": [ 1 ] ,'sClass': ' dt-body-left dt-head-left'}
                        ]);
                }
              },
        ...
        $(document).on('click', '.question-btn', function () {
            ...
            $.ajax({
                type: 'POST',
                url: baseurl + "onlinecourse/courseexamquestion/addform",
        ...
        $(document).on('click', '.question-btn-edit', function () {
            ...
            $.ajax({
                type: 'POST',
                url: baseurl + "onlinecourse/courseexamquestion/editform",
                data: {'recordid': recordid},
                dataType: 'JSON',
                beforeSend: function () {
                    $this.button('loading');
    ...
    $("form#formimportquestion").submit(function (e) {
     ...
     (function ($) {
        'use strict';
        $(document).ready(function () {
            initDatatable('all-list', 'onlinecourse/courseexamquestion/getDatatable', [],[], 100,
       ...
       $(document).on('click', '.deleteSelected', function () {
            ...
            if (array_delete.length === 0) {
                ...
                $.ajax({
                type: 'POST',
                url: baseurl + "onlinecourse/courseexamquestion/bulkdelete",
...
function getImages(page,query=""){
         $.ajax({
            type: "POST",
            url: baseurl+'onlinecourse/courseexamquestion/getimages',
 ...
 $(document).on('keyup', '#search_box', function (event) {
  ...
  $(document).on('click', '.page-link', function(){
    ...
    $(document).on('change','#question_type',function(){
      ...
      }else if($(this).val() == "descriptive" || $(this).val() == ""){
        $('.ans_true_false').hide();
        $('.ans_checkbox').hide();
        $('.option_list').hide();
        $('.ans').hide();
