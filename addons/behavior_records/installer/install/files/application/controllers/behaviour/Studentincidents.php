        ...
        if (!empty($students->data)) {
            foreach ($students->data as $student_key => $student) {
                if($this->rbac->hasPrivilege('behaviour_records_assign_incident', 'can_add')){
                    $viewbtn = "<a href='#' data-placement='top' data-student-id=".$student->id." data-toggle='modal' data-backdrop='static' class='btn btn-primary btn-xs assignstudent'  data-toggle='tooltip' title='".$this->lang->line('assign_incident')."'><i class='fa fa-plus'></i></a>";
                }
                if($this->rbac->hasPrivilege('behaviour_records_assign_incident', 'can_view')){
                    $assignedview = "<a href='#' data-placement='top' data-student-id=".$student->id." data-toggle='modal' data-backdrop='static' class='btn btn-primary btn-xs viewassignedincidents'  data-toggle='tooltip' title='".$this->lang->line('view_assigned_incidents')."'><i class='fa fa-reorder'></i></a>";
                }
                ...
                $row[] = "<div class='d-flex gap-0-5 text-right float-right'>".$viewbtn.' '. $assignedview."</div>" ;
