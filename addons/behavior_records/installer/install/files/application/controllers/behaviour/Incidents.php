        ...
        if (!empty($incidentlist->data)) {
            foreach ($incidentlist->data as $key => $value) {
                if ($this->rbac->hasPrivilege('behaviour_records_incident', 'can_edit')) {
                    $editbtn = "<a  class='btn btn-primary btn-xs editincidentmodel'  data-toggle='tooltip'   data-method_call='edit' data-original-title='" . $this->lang->line('edit') . "' data-record_id=".$value->id." ><i class='fa fa-pencil'></i></a>";
                }
                if ($this->rbac->hasPrivilege('behaviour_records_incident', 'can_delete')) {
                    $deletebtn = "<a href='#' data-record_id=".$value->id." class='btn btn-primary btn-xs deletebtn'  data-toggle='tooltip'  title='" . $this->lang->line('delete') . "' data-original-title='" . $this->lang->line('delete') . "'><i class='fa fa-remove'></i></a>";
                }
                ...
                $row[] = "<div class='d-flex gap-0-5 text-right float-right'>".$editbtn.' '. $deletebtn."</div>" ;
                $dt_data[] = $row;  
            }
