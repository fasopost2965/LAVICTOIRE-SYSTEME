<?php
/**
 * CE2 to CM1 Migration Script - V2
 */
...
foreach ($students as $index => $student) {
    ...
    // A. Detect Fee Base - More flexible LIKE
    $tarif_base = 0;
    $source_tarif = 'aucun';
    $fee_master_sql = "SELECT fgft.amount, fg.name 
                       ...
                       AND fg.name LIKE '%Scolarit%' 
                       AND (fg.name LIKE '%Primair%' OR fg.name LIKE '%Tarif%')
                       LIMIT 1";
    $fm_res = $conn->query($fee_master_sql);
    if ($fm_res && $fm_row = $fm_res->fetch_assoc()) {
    ...
    if ($detected_type == 'STANDARD_900') {
        ...
        $csv_data[16] = "Tarif non d??tect?? ou inconnu ($tarif_base) - Groupe: $source_tarif";
